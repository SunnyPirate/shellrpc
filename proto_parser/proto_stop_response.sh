#!/bin/bash
#[Proto] stop_response parser don't edit it

source ${rpc_lib_base_path}/rpc_common.sh
function proto_stop_response_get_result {
	local proto_data=${1}
	proto_stop_response_check_valid ${proto_data}
	if [ ${RPC_FAIL} -eq $? ];then
		RPCLog "[ERROR] proto stop_response unmatch version"
		echo ""
		return
	fi
...
function proto_stop_response_put_result {
	local proto_data=${1}
...
function proto_stop_response_make {
	local meta=f0543eff
...
function proto_stop_response_check_valid {
    local proto_data=${1}	
	local version_magic_num=${proto_data:0:8}
    if [ "f0543eff" == "${version_magic_num}" ];then
		return ${RPC_SUCCESS}
	else
        return ${RPC_FAIL}	
	fi
