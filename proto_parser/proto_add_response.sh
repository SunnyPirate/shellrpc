#!/bin/bash
#[Proto] add_response parser don't edit it

source ${rpc_lib_base_path}/rpc_common.sh
function proto_add_response_get_sum {
	local proto_data=${1}
	proto_add_response_check_valid ${proto_data}
	if [ ${RPC_FAIL} -eq $? ];then
		RPCLog "[ERROR] proto add_response unmatch version"
		echo ""
		return
	fi
...
function proto_add_response_put_sum {
	local proto_data=${1}
...
function proto_add_response_make {
	local meta=7e420b60
...
function proto_add_response_check_valid {
    local proto_data=${1}	
	local version_magic_num=${proto_data:0:8}
    if [ "7e420b60" == "${version_magic_num}" ];then
		return ${RPC_SUCCESS}
	else
        return ${RPC_FAIL}	
	fi
