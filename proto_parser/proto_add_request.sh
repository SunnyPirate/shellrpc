#!/bin/bash
#[Proto] add_request parser don't edit it

source ${rpc_lib_base_path}/rpc_common.sh
function proto_add_request_get_param1 {
	local proto_data=${1}
	proto_add_request_check_valid ${proto_data}
	if [ ${RPC_FAIL} -eq $? ];then
		RPCLog "[ERROR] proto add_request unmatch version"
		echo ""
		return
	fi
...
function proto_add_request_put_param1 {
	local proto_data=${1}
...
function proto_add_request_get_param2 {
	local proto_data=${1}
	proto_add_request_check_valid ${proto_data}
	if [ ${RPC_FAIL} -eq $? ];then
		RPCLog "[ERROR] proto add_request unmatch version"
...
function proto_add_request_put_param2 {
	local proto_data=${1}
	local proto_field_data=${2}
	proto_add_request_check_valid ${proto_data}
	if [ ${RPC_FAIL} -eq $? ];then
...
function proto_add_request_make {
	local meta=50036bae
...
function proto_add_request_check_valid {
    local proto_data=${1}	
	local version_magic_num=${proto_data:0:8}
    if [ "50036bae" == "${version_magic_num}" ];then
		return ${RPC_SUCCESS}
	else
        return ${RPC_FAIL}	
	fi
