#!/bin/bash
#[Proto] echo_response parser don't edit it

source ${rpc_lib_base_path}/rpc_common.sh
function proto_echo_response_get_message {
	local proto_data=${1}
	proto_echo_response_check_valid ${proto_data}
	if [ ${RPC_FAIL} -eq $? ];then
		RPCLog "[ERROR] proto echo_response unmatch version"
		echo ""
		return
	fi
...
function proto_echo_response_put_message {
	local proto_data=${1}
...
function proto_echo_response_make {
	local meta=20881b88
...
function proto_echo_response_check_valid {
    local proto_data=${1}	
	local version_magic_num=${proto_data:0:8}
    if [ "20881b88" == "${version_magic_num}" ];then
		return ${RPC_SUCCESS}
	else
        return ${RPC_FAIL}	
	fi
